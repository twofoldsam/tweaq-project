/**
 * CDP Helper for Runtime Signal Collection
 * 
 * This module provides Chrome DevTools Protocol utilities for collecting
 * runtime information about selected elements and source maps.
 */
 ...
 */
export function collectNodeSnapshot(element: Element): NodeSnapshot {
 ...
 */
async function fetchSourceMap(url: string): Promise<SourceMapInfo | null> {
  try {
 ...
 */
export async function collectSourceMaps(): Promise<SourceMapInfo[]> {
 ...
 */
export async function collectRuntimeSignals(element?: Element): Promise<RuntimeSignals> {
  const signals: RuntimeSignals = {
    sourcemaps: []
  };
  
  // Collect node snapshot if element is provided
  if (element) {
    signals.nodeSnapshot = collectNodeSnapshot(element);
  }
